...
const db = new mssql_1.ConnectionPool({
    ...
    connectionTimeout: config.get("connections.default.connectionTimeout"),
    requestTimeout: config.get("connections.default.requestTimeout"),
    options: {
        encrypt: config.get("connections.default.encrypt") // azure sqldatabse 需要设置为 true
    },
    pool: {
        max: 64,
        min: 1,
...
class DbUtil {
    ...
    static async executeSql(request, sql, retryTimes = 3) {
        return await retryUtil_1.RetryUtil.tryInvokeAsync(async () => {
    ...
    static async query(sql, params, retryTimes = 3) {
    ...
    ;
    static async add(addObj, tableName) {
        if (!addObj) {
            return null;
        }
    ...
    ;
    static async addIfNotExist(addObj, tableName, whereObj) {
        if (!addObj) {
            return null;
        }
        if (whereObj == null) {
    ...
    ;
    static async addList(addObjs, tableName) {
    ...
    ;
    static async addListIfNotExist(addObjs, uniqueFieldName, tableName) {
        if (!addObjs || addObjs.length == 0) {
            return null;
        }
        let addObj = addObjs[0];
    ...
    ;
    static async update(updateObj, whereObj, tableName) {
        var request = db.request();
